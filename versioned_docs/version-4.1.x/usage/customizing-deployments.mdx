...
It is recommended to edit the file located in the Config folder of the deployment template, rather than the copy located inside of the module itself.
...
- Both toolkit and MSI logs are temporarily placed in a temp folder, the path of which is set in `$adtSession.LogTempFolder` and defaults to `$envTemp\$installName`.
