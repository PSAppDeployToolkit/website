...
|:-----------------------------|:------------------------------|:------------------------------------------------------------------------------|
| `Deploy-Application.ps1`     | `Invoke-AppDeployToolkit.ps1` | [Invoke-AppDeployToolkit](../deployment-concepts/invoke-appdeploytoolkit) |
| `Deploy-Application.exe`     | `Invoke-AppDeployToolkit.exe` | [Invoke-AppDeployToolkit](../deployment-concepts/invoke-appdeploytoolkit) |
| `AppDeployToolkitConfig.xml` | `Config\Config.psd1`          | [Customizing Deployments](../usage/customizing-deployments)               |
| *Was part of Config file*    | `Strings\Strings.psd1`        | [Add correct link](../usage/customizing-deployments)                      |
...
For a detailed mapping of all v3 to v4 functions, check out the [Function Mapping](../reference/v4-function-mapping) page.
...
|:------------------|:--------------------------------------------------------------------|
| `Copy-File`       | [`Copy-ADTFile`](../reference/functions/Copy-ADTFile)           |
| `Execute-Process` | [`Start-ADTProcess`](../reference/functions/Start-ADTProcess)   |
| `Write-Log`       | [`Write-ADTLogEntry`](../reference/functions/Write-ADTLogEntry) |

## Variables

- The standard toolkit environment variables (e.g. `$envProgramFiles`) will be available once an [ADTSession](../reference/adtsession-object) has been started.

- Variables specific to a particular deployment (e.g. `$appName`) can now be found within the [ADTSession](../reference/adtsession-object) object (e.g. `$adtSession.AppName`).
...
- [`Uninstall-ADTApplication`](../reference/functions/Uninstall-ADTApplication) can now support uninstallation of **EXE** apps in addition to **MSI**.
