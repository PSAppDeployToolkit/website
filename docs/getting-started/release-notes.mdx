...
### v4.2.0 RC1 (2026-07-01)

#### Highlights

- Major code refactoring to clean up and optimise the code base.
- Module has reduced in size from 20.5MB to 13MB.
- iNKORE WPF library replaced by [Fluence](https://github.com/sintaxasn/Fluence.Wpf) (created and maintained by PSAppDeployToolkit founder Dan Cunningham).
...
- Dialogs now fallback to default image if specified asset is not found, also images can be encoded as Base64 strings instead of supplying file paths.
...
- Copy-ADTContentToCache now has a separate cache location for non-admins.
...
- [Uninstall-ADTApplication](../reference/functions/Uninstall-ADTApplication): added `-ForceUninstallString`
...
- With `DeployMode` left as `Auto`, sessions will now be silent by install by default if no processes to close are defined, unless `NoProcessDetection = $true` is set in the session properties.
