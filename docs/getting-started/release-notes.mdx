...
##### 🖥️ 4.1 RC3 Updates - 2025-07-30

- Changed default `-DeployMode` to `Auto`. This is functionally the same as the old `Interactive` mode, where the toolkit will automatically detect if a user is logged on and switch to Silent mode if no user is logged on.
- Changed `-DeployMode Interactive` to always show interactively. This means the toolkit will always show the dialogs, even if there are no processes to close.
- Added `$adtSSession.NoOobeDetection` which allows you to bypass OOBE detection if you want to run the toolkit in a non-OOBE environment.
- Added `$adtSSession.NoSessionDetection` which allows you to bypass session detection if you want to run the toolkit in a non-session environment.

##### 4.1 RC2 Updates - 2025-07-08

- Added [Test-ADTEspActive](../reference/functions/Test-ADTEspActive.mdx) to test for an active Enrollment Status Page (ESP).
...
- Up until now, it was not possible to display any user interface when deploying an application as SYSTEM using Intune (or any endpoint management tool) without using ServiceUI. Well, now it IS possible:
  ...
  - In fact, we strongly advise you stop using it as soon as po1sible. ServiceUI works by manipulating system security tokens in a way that could allow malicious actors to escalate privileges or bypass security controls.
...
- Furthermore, the Fluent UI has gained new features:
  ...
  - Custom accent colors via `UI.FluentAccentColor` configuration
...
- Added /Debug switch to [Invoke-AppDeployToolkit.exe](../deployment-concepts/invoke-appdeploytoolkit.mdx) to show terminal output for debugging purposes
- Added /Core switch to [Invoke-AppDeployToolkit.exe](../deployment-concepts/invoke-appdeploytoolkit.mdx) to allow PowerShell 7 usage
