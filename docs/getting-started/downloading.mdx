...
PSAppDeployToolkit.Tools is a companion module , designed for functions that will help you create your packages rather than deploy them. At present, this is in pre-release status, so installing from the PowerShell Gallery requires an additional switch:
