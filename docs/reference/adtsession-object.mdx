...
| `NoOobeDetection`     | If set to `$true`, bypasses OOBE detection logic for this deployment. Useful for advanced scenarios where you want to control DeployMode regardless of OOBE state.            |
| `NoSessionDetection`  | If set to `$true`, bypasses session detection logic for this deployment. Useful for advanced scenarios where you want to control DeployMode regardless of user session state. |
