...
description: 'Every property of the ADTSession object that governs a deployment.'
...
Why deployment state is held in a session object at all is discussed in
[Architecture](../explanation/architecture.mdx). In a deployment built from the template, these
properties are set in the `$adtSession` hashtable at the top of `Invoke-AppDeployToolkit.ps1`. See
