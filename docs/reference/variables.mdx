...
These variables can also be instantiated without opening a session by calling the [Export-ADTEnvironmentTableToSessionState](../reference/functions/Export-ADTEnvironmentTableToSessionState) command.

Variables specific to a particular deployment, such as `$appName` and `$appVersion`, have now been moved inside of the **ADTSession** object. For more information, see the [ADTSession Object](./adtsession-object) page.
