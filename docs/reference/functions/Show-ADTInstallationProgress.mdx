...
The `Show-ADTInstallationProgress` function creates a UI window in a separate thread to display a progress bar with a custom message that can be updated.
...
The default status message is taken from the imported `strings.psd1` file.
...
The percentage to display on the progress bar with a fluent progress window, between 0 and 100.
If null or not supplied, an indeterminate progress bar is displayed.
...
[https://github.com/PSAppDeployToolkit/PSAppDeployToolkit/blob/main/modules/PSAppDeployToolkit/Public/Show-ADTInstallationProgress.ps1](https://github.com/PSAppDeployToolkit/PSAppDeployToolkit/blob/main/modules/PSAppDeployToolkit/Public/Show-ADTInstallationProgress.ps1)
