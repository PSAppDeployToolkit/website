...
PSADT will handle this parameter automatically using the time of the installation as the version number.
    - In Windows 10, scripts and executables might be blocked by AppLocker.
Ensure that the path given to `-StubExePath` will permit end users to run scripts and executables unelevated.
