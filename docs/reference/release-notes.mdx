...
- Dialogs now fall back to default image if the specified asset is not found, also images can be encoded as Base64 strings instead of supplying file paths.
...
- #1893 [Feature] Improvements for `Uninstall-ADTApplication`
...
- #1948 [Bug] `Start-ADTProcess -ArgumentList` mishandles quotation marks when passed a string array
...
- #1980 [Bug] `Set-ADTIniValue` writes to new Section instead of updating the existing value
...
- #2021 [Bug] `Start-ADTProcessAsUser` runs while TaskSequence
...
- Added more error info to assist with troubleshooting an issue
...
- Changed `Start-ADTMsiProcessAsUser` to ensure it uses a user-writable log location.
...
- Fixed `CloseProcesses` issue introduced in 4.1.2
