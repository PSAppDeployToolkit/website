---
sidebar_position: 10
title: 'How to upgrade from v4.1 to v4.2'
description: 'Upgrade guidance for those coming from v4.1.'
slug: /how-to/upgrade-from-v4-1
hide_title: true
tags:
  - Migration
  - How-to
...
In v4.2, it will now switch to a silent install by default *even if no processes are defined*. To prevent this changing existing behavior on older packages, this will only apply if DeployAppScriptVersion is unset or >= 4.2.0.
...
- ZeroConfig code not included by default; there is a separate ZeroConfig template now that can be downloaded or generated via `New-ADTTemplate -ZeroConfig`.
